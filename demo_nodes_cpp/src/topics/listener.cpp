...
class ComplexListener : public rclcpp::Node
{
  using MessageT = test_msgs::msg::DynamicArrayPrimitives;

public:
  explicit ComplexListener(const std::string & topic_name)
  : Node("complex_listener")
  {
    // Create a callback function for when messages are received.
    // Variations of this function also exist using, for example UniquePtr for zero-copy transport.
    auto callback =
      [this](const MessageT::SharedPtr msg) -> void
      {
        for (auto i = 0u; i < msg->int32_values.size(); ++i) {
          fprintf(stderr, "%zd: %d\n", i, msg->int32_values[i]);
        }
        for (auto i = 0u; i < msg->string_values.size(); ++i) {
          fprintf(stderr, "%zd: %s\n", i, msg->string_values[i].c_str());
        }
      };
    ...
    // Note that not all publishers on the same topic with the same type will be compatible:
    // they must have compatible Quality of Service policies.
    sub_ = create_subscription<MessageT>(topic_name, callback);
  }
...
int main(int argc, char * argv[])
{
  ...
  //auto node = std::make_shared<ComplexListener>(topic);
