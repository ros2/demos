...
rclcpp::parameter::ParameterVariant parse_args(int argc, char **argv,
                                               std::string &remote_node,
                                               param_operation_t &op) {
  if (argc < 3) {
...
int main(int argc, char **argv) {
  ...
  if (op == PARAM_GET) {
    auto get_parameters_result =
        parameters_client->get_parameters({var.get_name()});
    ...
    if ((get_result != rclcpp::executor::FutureReturnCode::SUCCESS) ||
        ...
        (get_parameters_result.get()[0].get_type() ==
         rclcpp::parameter::PARAMETER_NOT_SET)) {
    ...
    if (list_result == rclcpp::executor::FutureReturnCode::SUCCESS) {
      ...
      RCLCPP_ERROR(node->get_logger(),
                   "Timed out trying to list parameters: 10 seconds")
      return 1;
    } else {
